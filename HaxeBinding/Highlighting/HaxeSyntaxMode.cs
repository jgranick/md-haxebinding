...
namespace MonoDevelop.HaxeBinding.Highlighting
{
	public class HaxeSyntaxMode : SyntaxMode
	{
		static SyntaxMode baseMode;

		public HaxeSyntaxMode ()
		{
			...
			if (baseMode == null)
			{
				var provider = new ResourceStreamProvider(
					typeof(HaxeSyntaxMode).Assembly,
					typeof(HaxeSyntaxMode).Assembly.GetManifestResourceNames().First(s => s.Contains("HaxeSyntaxHighlightingMode")));
